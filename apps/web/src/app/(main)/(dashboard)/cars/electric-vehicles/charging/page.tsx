...
async function ChargingBento({ searchParams }: PageProps) {
  const [{ district, power }, summary] = await Promise.all([
    loadSearchParams(searchParams),
    getEvChargingLiveSummary(),
  ]);

  if (summary.connectors === 0) {
    return (
      <EmptyState
        description="Live charger availability has not been collected yet. Please check back later."
        icon={
