...
Uses **Recharts** via the local `ChartContainer` wrapper for data visualization. Always use CSS variables for colors (see `design-language-system` skill).

## Core Pattern

All charts use `ChartContainer` from `@web/components/charts/chart` wrapped in `ChartWidget` from `@web/components/charts/widget`. **Never use raw `ResponsiveContainer` directly.**
...
Recharts `ResponsiveContainer` logs `width(-1) and height(-1)` warnings on first render before `ResizeObserver` fires. Fix by setting `initialDimension` on `ResponsiveContainer` in `apps/web/src/components/charts/chart.tsx`:
...
Reference: [Recharts #6716](https://github.com/recharts/recharts/issues/6716)
...
- Chart component source: `apps/web/src/components/charts/chart.tsx`
